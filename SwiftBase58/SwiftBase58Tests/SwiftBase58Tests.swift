...
class SwiftBase58Tests: XCTestCase {
    ...
    func testEncodeBase58() {
        ...
        for (testNum, (inString, outString)) in stringTests.enumerated() {
    ...
    func testDecodeBase58() {
        do {
            for (testNum, (inString, outString)) in hexTests.enumerated() {
                
                let stringBuf   = try SwiftHex.decodeString(hexString: inString)
    ...
    func testInvalidDecodeBase58() {
        for (testNum, (inString, outString)) in invalidStringTests.enumerated() {
            let result = SwiftBase58.decode(inString)
            let resString = String(bytes: result, encoding: String.Encoding.utf8)!
            
            XCTAssertEqual(resString, outString, "Base58.decode invalidString test #\(testNum+1) failed! Got: \(resString) wanted: \(outString)")
    ...
    func testPerformanceDecodeShort() {
        ...
        self.measure() {
            ...
            let _ = SwiftBase58.decode(input)
        }
    }
    
    func testPerformanceEncodeShort() {
        let byteInput = [UInt8]("1NS17iag9jJgTHD1VXjvLCEnZuQ3rJDE9L".utf8)
        self.measure() {
            // Put the code you want to measure the time of here.
            let _ = SwiftBase58.encode(byteInput)
        }
    }
